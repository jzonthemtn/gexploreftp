...
namespace Assemblies.Ftp.FileSystem.Interfaces.AWS
{
	class S3FileSystemObject : IFileSystem
	{
        ...
        public S3FileSystemObject(string sStartDirectory)
		{
		...
		{
            Standard.StandardFileObject file = new Standard.StandardFileObject(GetPath(sPath), fWrite);
		...
		{
            Standard.StandardFileInfoObject info = new Standard.StandardFileInfoObject(GetPath(sPath));
			if (info.Loaded)
			{
				return info;
			}
