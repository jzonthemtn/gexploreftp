...
namespace gExploreFTP.FTP.FtpServer
{
	...
	class FtpConnectionObject : FtpConnectionData
	{
        ...
        public FtpConnectionObject(FileSystem.Interfaces.IFileSystemClassFactory fileSystemClassFactory, int nId, System.Net.Sockets.TcpClient socket)
			: base(nId, socket)
		{
		...
		{
            FileSystem.Interfaces.IFileSystem fileSystem = m_fileSystemClassFactory.Create(this.User, sPassword);
		...
		private void LoadCommands()
		{
            ...
            AddCommand(new FtpCommands.CDUPCommandHandler(this));
			AddCommand(new FtpCommands.PortCommandHandler(this));
			AddCommand(new FtpCommands.PasvCommandHandler(this));
            AddCommand(new FtpCommands.SYSTCommandHandler(this));
		...
		public void Process(Byte [] abData)
		{
			...
			if (handler == null)
			{
                ...
                gExploreFTP.FTP.General.SocketHelpers.Send(Socket, "550 Unknown command\r\n");
			}
			else
			{
